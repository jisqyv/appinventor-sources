...
//      out.println("");
  ...
  private boolean memcacheNotAvailable() {
    ...
    try {
      status = service.getStatus(Capability.MEMCACHE).getStatus();
    } catch (Exception e) {     // Probably running on stand alone server
      LOG.log(Level.WARNING, "Exception checking memcache status, ignore this if on standalone server", e);
      return true;
    }
